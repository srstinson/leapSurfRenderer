...
public:
    ...
    void updateStateStr()
    {
        ...
        m_aStrState[1]  << "[N]ext Mode: "    << mode;
		m_aStrState[2]  << "Dollar Recognizer Sez: " << *new String(dollarGestureName.c_str());
    ...
    void update( Leap::Frame frame )
    {
		...
		//send current coord to the Gesture Recognition Engine
		recognitionEngine->updateCoord(MotionVisualizerApplication::getController());
		dollarGestureName = recognitionEngine->recognize().name;
...
void MotionVisualizerApplication::initialise (const String& commandLine)
{
	...
	recognitionEngine = DollarRecognizer::GestureRecognitionEngine::Instance();
